 ...
 *   Box 188 — Marginal relief (if profits £50k-£250k)
...
var CT_RATES = {
  SMALL_RATE:       0.19,   // 19% — profits up to £50k
  MAIN_RATE:        0.25,   // 25% — profits over £250k
  SMALL_LIMIT:      50000,
  UPPER_LIMIT:      250000,
...
function _computeCT(profits) {
  ...
  // Marginal relief band — £50k to £250k
...
function _getCapitalAllowances(ss, periodStart, periodEnd) {
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      total += cost; // simplified — full AIA assumed (£1m limit from 2023)
    }
    return total;
  } catch(e) {
